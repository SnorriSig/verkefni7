...
function play() {
  ...
  do {
    let input = prompt("Giskaðu á tölu sem er á milli 0 og 100");

    if (input === null) {
      window.alert('hætt í leik');
      games.push(numbergames++);
      break;
    }
...
function calculateAverage() {
  ...
  return avgGuesses = (guessCount / games.length);
}
...
function parseGuess(input) {
  if (isNaN(parseInt(input))) {
    let parserguess = null;
  }
  else {
    var parserguess = parseInt(input);
  }
  return parserguess;
}
